    ...
    listing:
      - entryname: select_pass.sh
        entry: |
          ...
          if [[ $(inputs.mode) = gatk ]]
          then
            /gatk SelectVariants --java-options "-Xmx8000m" --exclude-filtered TRUE -V $(inputs.input_vcf.path) -O $(inputs.output_basename).$(inputs.tool_name).PASS.vcf
          elif [[ $(inputs.mode) = grep ]]
          then
            zcat $(inputs.input_vcf.path) | grep -E "^#|PASS"  > $(inputs.output_basename).$(inputs.tool_name).PASS.vcf
          else
