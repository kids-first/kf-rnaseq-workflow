...
Users are encouraged to provide read group strings that correspond to their


The workflow is designed to handle multiple read groups; however, it cannot handle
a mix of single end and paired end reads.



The reads record also has fields for metadata relevant to that read group. Use
these fields as necessary:
- `is_paired_end`: Are the reads paired end?
- `cram_reference`: If inputting CRAM, provide the reference used to create the CRAM
- `outSAMattrRGline`: Provide the SAM specification `@RG` line for this read group. Suggested setting format is: ID:sample_name LB:aliquot_id PL:platform SM:BSID for example ID:7316-242 LB:750189 PL:ILLUMINA SM:BS_W72364M
...
- `quality_cutoff`: Quality trim cutoff, see https://cutadapt.readthedocs.io/en/v3.4/guide.html#quality-trimming for how 5' 3' is handled
  ...
  input_alignment_files: { type: 'File[]?', secondaryFiles: [{"pattern": "^.bai", required: false }, {"pattern": ".bai", required: false }, {"pattern": "^.crai", required: false }, {"pattern": ".crai", required: false}], doc: "List of input SAM/BAM/CRAM files to process" }
    ...
    type:
      ...
      items:
        ...
        fields:
          ...
          cram_reference: { type: 'File?', secondaryFiles: [.fai], doc: "If input reads are CRAM, provide the reference used to create the " }
          ...
          quality_base: { type: 'int?', doc: "Phred scale used" }
...
```yaml
inputs:
  output_basename: "String to use as basename for outputs. Will use read1 file basename if null"
  reads1: "Input fastq file, gzipped or uncompressed OR alignment file file"
  reads2: "If paired end, R2 reads files, gzipped or uncompressed"
  ...
  gtf_anno: "General transfer format (gtf) file with gene models corresponding to fasta reference"
  star_fusion_genome_untar_path: "This is what the path will be when genome_tar is unpackaged"
  reference_fasta: "Reference FASTA, and associated FAI index, to which reads will be aligned"
```
